...
public:
  ...
  Date(int d=1, int m=1, int y=1990); // nowy konstruktor domyœlny
  Date( const Date& d ); // konstruktor kopiuj¹cy
  Date& operator = (const Date& d );
  ~Date(); // destruktor
  friend TESTDLL_API istream& operator >> (istream& in, Date& d ); // wejœciowy, d oraz in to parametry wejsc/wyjsc
